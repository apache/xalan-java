...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- An XSL named template returning different function items, depending 
       ...
       xsl:call-template. This template doesn't validate the return value of 
       template's result with an XPath sequence type (since there's no "as" 
       attribute on xsl:template element. -->
  <xsl:template name="getFuncItem">
