...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- An XSL 3.0 test case for the, XPath 3.1 function array:fold-right.
    ...
    <xsl:template match="/">
	   ...
	   <result>
	     <one>
	       <xsl:value-of select="array:fold-right($arr1, true(), $func1)"/>
	     </one>
	     <two>
	       <xsl:value-of select="array:fold-right($arr1, false(), $func2)"/>
	     </two>
	   </result>
    </xsl:template>
