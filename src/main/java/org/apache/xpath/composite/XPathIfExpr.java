...
public class XPathIfExpr extends Expression {
    ...
    public XObject execute(XPathContext xctxt) throws TransformerException {
       ...
       List<XMLNSDecl> nsAddedList = new ArrayList<XMLNSDecl>();
       ...
       try {    	   
    	   ...
    	   Hashtable nsHashtable = XslTransformData.m_ns_hashtable;

    	   if (nsHashtable != null) {
    		   nsHashtable.forEach((key, value) -> {
    			   String uri = (String)key;
    			   String prefix = (String)value;

    			   if (!prefixTableContains(prefixTable, prefix, uri)) {
    				   XMLNSDecl xmlNSDecl = new XMLNSDecl(prefix, uri, false);
    				   prefixTable.add(xmlNSDecl);

    				   nsAddedList.add(xmlNSDecl);
    			   }
    		   });
    		   ...
    		   if (exprNode instanceof ElemTemplateElement) {
    			   ElemTemplateElement elemTemplateElem = (ElemTemplateElement)exprNode; 
    			   elemTemplateElem.setPrefixTable(prefixTable);
    		   }
    	   }              
    	   ...
    	   boolean ifBranchConditionEmptySeq = ((ifConditionXPathResult instanceof ResultSequence) && 
    			              											(((ResultSequence)ifConditionXPathResult).size() == 0)) ? true : false;  
    	   ...
    	   if ((ifConditionEagerCheck && ifConditionEvalResult) || (!ifConditionEagerCheck && ifConditionXPathResult.bool())) {
    		   if (prefixTable != null) {
    			   m_thenExprXPathStr = XslTransformEvaluationHelper.replaceNsUrisWithPrefixesOnXPathStr(m_thenExprXPathStr, prefixTable);
    		   }
    		   ...
    		   if (prefixTable != null) {
    			   m_elseExprXPathStr = XslTransformEvaluationHelper.replaceNsUrisWithPrefixesOnXPathStr(m_elseExprXPathStr, prefixTable);
    		   }
    	   ...
    	   if (nsAddedList.size() > 0) {
        	   prefixTable.removeAll(nsAddedList);
           } 
       }
