 ...
 * @xsl.usage advanced
 */
public class VcGt extends XPathRelationalOp
{
  ...
  {      	  
	  ...
	  if (left instanceof ElemFunctionItem) {
		 throw new javax.xml.transform.TransformerException("FOTY0013 : An XPath 3.1 operator gt's first operand is a function type, "
		 		                                                                                                          + "which cannot be atomized.", srcLocator);   
	  }
	  
	  if (right instanceof ElemFunctionItem) {
		 throw new javax.xml.transform.TransformerException("FOTY0013 : An XPath 3.1 operator gt's second operand is a function type, "
		 		                                                                                                          + "which cannot be atomized.", srcLocator);   
	  }
	  ...
	  if (left instanceof ResultSequence) {
		  if (((ResultSequence)left).size() == 0) {
			  ...
			  throw new javax.xml.transform.TransformerException("FOTY0013 : An XPath 3.1 operator gt's first operand cannot be a "
	  ...
	  if (isLEmpty || isREmpty) {
		 // An XPath 3.1 operator 'gt' has one or both of it's operands as empty sequence
		 result = new ResultSequence(); 
	  }
	  else {
		 result = left.vcGreaterThan(right, getExpressionOwner(), xctxt.getDefaultCollation(), true) ? XBoolean.S_TRUE : XBoolean.S_FALSE;  
	  }
