 ...
 * @xsl.usage advanced
 */
public class VcGe extends XPathRelationalOp
{
  ...
  {      
      ...
      if (left instanceof XPathMap) {
		  throw new javax.xml.transform.TransformerException("FOTY0013 : An xdm atomic value is required for the first operand of 'ge', but "
																												  + "the supplied type is a map "
																												  + "type which cannot be atomized.", srcLocator); 
	  }

	  if (right instanceof XPathMap) {
		  throw new javax.xml.transform.TransformerException("FOTY0013 : An xdm atomic value is required for the second operand of 'ge', but "
																												  + "the supplied type is a map "
																												  + "type which cannot be atomized.", srcLocator); 
	  }
	  ...
	  if (left instanceof ResultSequence) {
		  if (((ResultSequence)left).size() == 0) {
			  ...
			  throw new javax.xml.transform.TransformerException("FOTY0013 : An XPath 3.1 operator ge's first operand cannot be a "
	  ...
	  if (right instanceof ResultSequence) {
		  if (((ResultSequence)right).size() == 0) {
			  ...
			  throw new javax.xml.transform.TransformerException("FOTY0013 : An XPath 3.1 operator ge's second operand cannot be a "
			  																											 + "sequence of size greater than one.", srcLocator);
		  }
	  }
		  ...
		  if (nodeHandle != DTM.NULL) {
			  ...
			  if (node instanceof ElementPSVI) {
				  ...
				  if (typeDefn instanceof XSComplexTypeDefinition) {
					  throw new javax.xml.transform.TransformerException("FOTY0013 : An XPath 3.1 operator 'ge' operand, cannot be a "
	  ...
	  if ((XMLConstants.W3C_XML_SCHEMA_NS_URI).equals(typeNs1) && (XMLConstants.W3C_XML_SCHEMA_NS_URI).equals(typeNs2)) {
		  if ((isXsBuiltInTypeNumeric(typeName1) && !isXsBuiltInTypeNumeric(typeName2)) || 
			  ...
			  throw new javax.xml.transform.TransformerException("FOTY0013 : An XPath 3.1 operator 'ge' cannot, compare values of schema "
	  ...
	  if (isLEmpty || isREmpty) {
		 // An XPath 3.1 operator 'ge' has one or both of it's operands as empty sequence
		 result = new ResultSequence(); 
	  }
	  else {
		 result = left.vcLessThan(right, getExpressionOwner(), xctxt.getDefaultCollation(), false) ? XBoolean.S_FALSE : XBoolean.S_TRUE;  
	  }
