...
public class FuncMax extends FunctionMultiArgs
{
	...
	{          
		...
		if (xObjArg0 instanceof XMLNodeCursorImpl) {
			...
			for (int idx = 0; idx < size1; idx++) {
				...
				if (seqObj instanceof XSUntypedAtomic) {
					convetedInpSequence.add(new XSDouble(((XSUntypedAtomic)seqObj).stringValue()));
					doubleItemCount++;
				}
		...
		if (result == null) {
			if ((doubleItemCount > 0) && (convetedInpSequence.size() == doubleItemCount)) {
				...
				result = getMaxValueFromXSStringSequence(convetedInpSequence, collationUri, xctxt);
			}
	 ...
	 */
	private XSString getMaxValueFromXSStringSequence(ResultSequence inpSeq, String collationUri, 
	 ...
	 */
	private String getCollationUri(XPathContext xctxt) throws javax.xml.transform.TransformerException {
