...
/**
 * Implementation of XPath 3.1 function math:cos.
 * 
...
public class FuncMathCos extends FunctionOneArg
{
    ...
    public XObject execute(XPathContext xctxt) throws javax.xml.transform.TransformerException
    {
    	...
    	if (m_arg0 == null) {
    		throw new TransformerException("XPST0017 : An XPath 3.1 function call math 'cos' has been called with no argument.", srcLocator);
    	}
    	else if (m_arg0 instanceof NodeTest) {
    		if (XslTransformEvaluationHelper.isNodeTestExpressionFuntionType((NodeTest)m_arg0)) {
    			throw new javax.xml.transform.TransformerException("FOTY0013 : An xdm atomic value is required for the first argument of XPath 3.1 function math 'cos', "
																								    					+ "but the supplied type is a "
																								    					+ "function type, which cannot be atomized.", srcLocator); 
    		}
    	}
    	else if (m_arg0 instanceof XPathInlineFunction) {
    		throw new javax.xml.transform.TransformerException("FOTY0013 : An xdm atomic value is required for the first argument of XPath 3.1 function math 'cos', but the "
        ...
        try {
        	...
        	throw new TransformerException("XPTY0004 : An XPath 3.1 function call math 'cos' argument is not an XML Schema type double.", srcLocator);
        }
        
        if (arg0Obj instanceof XNumber) {
        	double dbl = Math.cos(((XNumber)arg0Obj).num());
        	...
        	double dbl = Math.cos((new XSDouble(strVal)).doubleValue());
        	...
        	if (xmlNodeCursorImpl.getLength() == 0) {
        		...
        		result = new XSDouble(Math.cos(dbl));
        	}
        }
            ...
            if (rSeq.size() == 0) {
               ...
               result = new XSDouble(Math.cos(dbl));
         	   ...
         	   throw new javax.xml.transform.TransformerException("XPTY0004 : An XPath 3.1 function math 'cos', requires an argument which "
