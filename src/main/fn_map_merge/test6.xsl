...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- An XSLT stylesheet test, to test the XPath 3.1 map:merge() 
         ...
         key duplicates resolution policy as 'reject' (i.e, if any map
         key occurs, within more than one input map, the stylesheet raises
         an error).
    ...
    <xsl:template match="/">
	   ...
	   <xsl:variable name="map2" select="map{5 : 'Sonnabend'}"/>
	   <xsl:variable name="mapSeq1" select="($map1,$map2)"/>
	   <xsl:variable name="options" select="map {'duplicates' : 'reject'}"/>
