...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="/elem">      
      ...
      <xsl:if test="count($result/yes) = count((test1 | test2)/a)">
         <xsl:text>all the xml elements '(test1 | test2)/a' contain an xml element 'e1'</xsl:text><xsl:text>&#xa;</xsl:text>
      </xsl:if>
      
      <!-- in-fact, we don't necessarily need to use xsl:variable to do this work. the 
           following XPath expression, works fine as well. -->  
      <xsl:if test="count((test1 | test2)/a) = count((test1 | test2)/a[e1])">
         <xsl:text>all the xml elements '(test1 | test2)/a' contain an xml element 'e1'</xsl:text>
      </xsl:if>
   </xsl:template>
