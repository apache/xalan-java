...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!--  An XSL 3 stylesheet test case, to test xsl:template match 
		...
		XDM atomic value to the called template. This stylesheet, 
		gets data to be processed from an XML external document. -->			      

  <xsl:output method="xml" indent="yes"/>

  <xsl:template match="/info">
      <result>
	     <xsl:variable name="seq1" select="(xs:integer(value[1]), xs:date(value[2]), xs:date(value[3]), xs:integer(value[4]))"/>
