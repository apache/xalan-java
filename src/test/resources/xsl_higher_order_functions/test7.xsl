<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">     
    <result>
      <xsl:value-of select="fn0:calc1($sqr1, info/val1, info/val2)"/>
    </result>
  </xsl:template>
  
  <!-- A stylesheet function that accepts three arguments,
       one of which is a function item, and the other two
       are integer values.
  -->
  <xsl:function name="fn0:calc1" as="xs:integer">
     ...
     <xsl:sequence select="$sqr1($val1) * $val2"/>
  </xsl:function>
