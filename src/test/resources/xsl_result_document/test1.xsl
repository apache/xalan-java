...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"                               
  ...
  <xsl:template match="/">
	 ...
	 <xsl:result-document href="credits.xml" method="xml">
		<credits>
		   <xsl:for-each select="$txnXmlDocNode/transactions/transaction[@value &gt;= 0]">
			 <xsl:copy-of select="."/>
		   </xsl:for-each>
		</credits>
	 </xsl:result-document>
	 <xsl:result-document href="debits.xml" method="xml">
		<debits>
		   <xsl:for-each select="$txnXmlDocNode/transactions/transaction[@value &lt; 0]">
			  <xsl:copy-of select="."/>
		   </xsl:for-each>
