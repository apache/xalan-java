...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- An XSLT stylesheet test, to test the XPath 3.1 map:merge() 
         ...
         default options (that has, map keys duplicate resolution policy
         as use-first).
    ...
    <xsl:template match="/">
	   <xsl:variable name="map1" select="map:entry(0,'no')"/>
	   <xsl:variable name="map2" select="map:entry(1,'yes')"/>
	   <xsl:variable name="mapSeq1" select="($map1,$map2)"/>
	   <xsl:variable name="mapAfterMerge" select="map:merge($mapSeq1)"/>
