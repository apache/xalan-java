...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <!-- An XSL 3 stylesheet test case, to test XPath 3.1 collection function 
         ...
         information from various CSV document files into an aggregate XML 
         result document. -->				
		  ...
		  <xsl:for-each select="1 to $noOfCsvDocuments">
			 ...
			 <xsl:for-each select="1 to $noOfLinesInFile">
				...
				<xsl:if test="$lineNo gt 1">
			      <row>
				    <id><xsl:value-of select="$tokenSeq[1]"/></id>
				    <employeeId><xsl:value-of select="$tokenSeq[2]"/></employeeId>
				    <employeeName><xsl:value-of select="$tokenSeq[3]"/></employeeName>
				    <departmentId><xsl:value-of select="$tokenSeq[4]"/></departmentId>
			      </row>
				</xsl:if>
			 </xsl:for-each>
		  </xsl:for-each>
