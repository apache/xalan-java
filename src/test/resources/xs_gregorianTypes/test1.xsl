...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
	   <result>
	      <xsl:variable name="val1" select="xs:gYearMonth('2000-10')"/>
		  <xsl:variable name="val2" select="xs:gYearMonth('2005-10')"/>
		  <one><xsl:value-of select="$val1 eq $val1"/></one>
		  <two><xsl:value-of select="$val1 ne $val2"/></two>
		  <xsl:variable name="val3" select="xs:gYear('2007')"/>
		  <xsl:variable name="val4" select="xs:gYear('2007')"/>
