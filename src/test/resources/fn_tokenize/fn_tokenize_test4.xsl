  ...
  <xsl:template match="/">     
     <result>
		 ...
		 <xsl:call-template name="emitTokens">
		    <xsl:with-param name="tokens" select="tokenize($str1, '\s+')"/>
		 </xsl:call-template>
	 </result>
  </xsl:template>
  
  <!-- An XSL stylesheet named template, that accepts a sequence of tokens
       and emits them to stylesheet's output. -->
