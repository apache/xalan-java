...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
     <details>
	    <xsl:variable name="fnTransformArg" select="fn0:getFnTransformArg('render.xsl', doc('test1.xml'))" as="map(*)"/>	 
  ...
  <!-- An XSL function definition, to get function fn:transform's 
       ...
       stylesheet-location option while using function fn:transform. 
  ...
  <xsl:function name="fn0:getFnTransformArg" as="map(*)">
     <xsl:param name="stylesheetUrl" as="xs:string"/>
	 <xsl:param name="xmlInpNode" as="node()"/>
	 <xsl:variable name="fnTransformArg" select="map {}"/> 
	 <xsl:variable name="fnTransformArg" select="map:put($fnTransformArg, 'stylesheet-location', $stylesheetUrl)"/>
