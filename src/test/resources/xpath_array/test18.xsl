...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
       ...
       <xsl:variable name="arr2" select="array:tail($arr1)" as="array(xs:string)"/>
       <xsl:variable name="func1" select="function($x, $y) { $x || $y }"/>
       <xsl:variable name="resultArr1" select="array:for-each-pair($arr1, $arr2, $func1)" as="array(xs:string)"/>       
       <result>
