...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="/" xpath-default-namespace="http://ns0">
      <result>
	     <one>
   ...
   <xsl:function name="fn0:getElemNodeStringLength1" as="xs:integer" xpath-default-namespace="http://ns0">
      <xsl:param name="p1" as="element()"/>
	  <xsl:sequence select="string-length($p1/self::subitem)"/>
   </xsl:function>
   
   <xsl:function name="fn0:getElemNodeStringLength2" as="xs:integer" xpath-default-namespace="http://ns0">
      <xsl:param name="p1" as="element()"/>
