...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:function name="fn0:getElemNodeStringLength1" as="xs:integer">
	  ...
	  <xsl:sequence select="string-length($p1/self::subitem)" xpath-default-namespace="http://ns0"/>
   </xsl:function>
   
   <xsl:function name="fn0:getElemNodeStringLength2" as="xs:integer">
      <xsl:param name="p1" as="element()"/>
	  <xsl:sequence select="string-length($p1/self::substr)" xpath-default-namespace="http://ns0"/>
   </xsl:function>
