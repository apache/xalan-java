...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
	 <result>
	    <xsl:variable name="var1" select="'thanks x foR Reading y. p and q and r.'"/>
		<xsl:variable name="var2" select="'thanks x foR Reading y. p and q and r.'" as="xs:string"/>
	    <one>
		   <xsl:value-of select="'thanks x foR Reading y. p and q and r.'"/>
		</one>
		<two>
		   <xsl:value-of select="xs:string('thanks x foR Reading y. p and q and r.')"/>
		</two>
		<three>
